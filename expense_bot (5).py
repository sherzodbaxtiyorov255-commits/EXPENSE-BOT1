...
async def account_management_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "acctmgmt:edit":
        ...
        buttons = [[InlineKeyboardButton(f"✏️ {n}", callback_data=f"acctrename:{n}")] for n in names]
        await query.edit_message_text(
            "✏️ Qaysi hisobni tahrirlamoqchisiz? (nomini o'zgartirish uchun tanlang)",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        return
...
async def reset_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "⚠️ <b>Diqqat!</b> Bu barcha xarajat/daromad, qarz va hisoblar tarixini "
        "butunlay o'chiradi. Rostdan ham tozalashni xohlaysizmi?",
        reply_markup=InlineKeyboardMarkup(buttons),
    )
...
async def reset_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "reset:confirm":
        space_key = get_space_key_for_update(update)
        reset_space(space_key)
        await query.edit_message_text("Balans va tarix tozalandi. 🧹")
    else:
...
async def process_free_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, silent_if_unrecognized: bool = False):
    ...
    # 1e) Yangi hisob qo'shish kutilayotgan bo'lsa
    if context.chat_data.get("pending_new_account"):
        ...
        if ok:
            await update.message.reply_text(f"✅ Yangi hisob qo'shildi: <b>{esc(name)}</b>\n\n{build_accounts_summary_text(space_key)}")
        else:
            await update.message.reply_text(f"Bu nomdagi hisob allaqachon mavjud: {esc(name)}")
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(account_management_callback, pattern=r"^acctmgmt:|^acctrename:"))
