...
def build_balance_view_text(space_key: str) -> str:
    ...
    lines = ["💸 Xarajatlar:"]
    total_line = f"  Umumiy: {fmt_num(total_exp['UZS'])} so'm"
    if total_exp["USD"] > 0:
        total_line += f", {fmt_num(total_exp['USD'], 2)} $"
    lines.append(total_line)

    today_line = f"  Bugungi: {fmt_num(today_exp['UZS'])} so'm"
    if today_exp["USD"] > 0:
        today_line += f", {fmt_num(today_exp['USD'], 2)} $"
...
def build_accounts_summary_text(space_key: str) -> str:
    ...
    for name, balance in rows:
        ...
        if exp_today > 0:
            lines.append(f"  {marker}{name}: {fmt_num(balance)} so'm  (bugun xarajat: {fmt_num(exp_today)} so'm)")
        else:
            lines.append(f"  {marker}{name}: {fmt_num(balance)} so'm")
    lines.append("  —")
...
START_TEXT = "Salom! Men sizning hisobchingizman. 🧮"

HELP_TEXT = (
    "Men xarajat/balans botiman.\n\n"
...
async def calendar_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    now = datetime.now()
    await update.message.reply_text(
        "📅 Hisobot olish uchun kunni tanlang:",
...
def build_day_report_text(space_key: str, date_str: str) -> str:
    ...
    if not rows:
        lines = [f"📅 {pretty_date}: bu kunda xarajat yo'q."]
    else:
        income = {"UZS": 0.0, "USD": 0.0}
        expense = {"UZS": 0.0, "USD": 0.0}
        lines = [f"📅 {pretty_date} bo'yicha hisobot:", ""]
        for amount, currency, category, note, created_at in rows:
            time_str = created_at.split("T")[1][:5] if "T" in created_at else ""
            sign = "+" if amount >= 0 else ""
            unit = "so'm" if currency == "UZS" else "$"
            lines.append(f"  {time_str}  {sign}{fmt_num(amount)} {unit}  [{category}] — {note}")
            if amount >= 0:
                income[currency] += amount
            else:
                expense[currency] += -amount
    ...
    lines.append("")
    lines.append(f"🏦 {pretty_date} kuni oxiridagi hisoblar qoldig'i:")
    total = 0.0
    for name in ACCOUNT_NAMES:
        bal = balances.get(name, 0.0)
        total += bal
        lines.append(f"  {name}: {fmt_num(bal)} so'm")
    lines.append(f"  Umumiy: {fmt_num(total)} so'm")
