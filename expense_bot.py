...
def process_morning_check_reply(user_id: int, space_key: str, text: str):
    ...
    for line in lines_in:
        ...
        if abs(diff) > 0.5:
            ...
            results.append(f"  {name}: kutilgan {expected:,.0f}, haqiqiy {actual:,.0f} — "
                            f"farq {abs(diff):,.0f} so'm [{category}] sifatida yozildi")
        else:
            results.append(f"  {name}: mos keladi ({actual:,.0f} so'm)")
