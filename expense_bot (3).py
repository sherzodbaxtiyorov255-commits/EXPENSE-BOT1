...
print(f"[STATUS] OCR_AVAILABLE={OCR_AVAILABLE}, MATPLOTLIB_AVAILABLE={MATPLOTLIB_AVAILABLE}, VOICE_AVAILABLE={VOICE_AVAILABLE}")
...
def build_accounts_summary_text(space_key: str) -> str:
    ...
    lines = ["💳 Hisoblar bo'yicha saldo:"]
    for name, balance in rows:
        lines.append(f"  {name}: {balance:,.0f} so'm")
    return "\n".join(lines)
...
async def show_method_totals(update: Update, context: ContextTypes.DEFAULT_TYPE, method: str, title: str):
    ...
    lines = [f"{title}:"]
    lines.append(f"  UZS: {totals['UZS']:,.0f} so'm")
    ...
    if recent:
        ...
        for amount, currency, category, note, created_at in recent:
            ...
            unit = "so'm" if currency == "UZS" else "$"
            lines.append(f"  {date_str}  {abs(amount):,.0f} {unit}  [{category}] — {note}")
    else:
        lines.append("")
...
async def saldo_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    space_key = get_space_key_for_update(update)
    context.user_data["pending_saldo_input"] = True
    await update.message.reply_text(
...
async def commit_expense(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, space_key: str,
    ...
    if amount < 0 and not account:
        context.user_data["pending_account_tx"] = {
...
async def _finalize_commit(update: Update, user_id: int, space_key: str, amount: float, currency: str,
    ...
    if account:
        apply_account_delta(space_key, account, amount)

    sign = "+" if amount >= 0 else ""
    unit = "so'm" if currency == "UZS" else "$"
    text = f"Qabul qilindi: {sign}{amount:,.0f} {unit}  [{category}]"
    if account:
        text += f"  ({account})"
...
async def account_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    data = query.data
    pending = context.user_data.pop("pending_account_tx", None)
    if not pending:
...
async def process_free_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    # 1b) Hisob saldosini kiritish kutilayotgan bo'lsa
    if context.user_data.get("pending_saldo_input"):
        ...
        context.user_data.pop("pending_saldo_input", None)
        await update.message.reply_text(f"{name}: {value:,.0f} so'm qilib belgilandi ✅\n\n{build_accounts_summary_text(space_key)}")
        return
...
def main():
    ...
    if app.job_queue is not None:
        try:
            ...
            print("Kunlik xabar (23:59, Toshkent vaqti) sozlandi.")
        except Exception as e:
            print(f"[OGOHLANTIRISH] Kunlik xabarni sozlab bo'lmadi: {e}")
    else:
