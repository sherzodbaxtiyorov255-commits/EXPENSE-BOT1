...
MAIN_KEYBOARD = ReplyKeyboardMarkup(
    [
        ...
        [BTN_DEBTS, BTN_KURS],
        [BTN_CARD, BTN_CASH],
        [BTN_SALDO, BTN_MANAGE_ACCOUNTS],
        [BTN_CALENDAR, BTN_EXCEL],
        [BTN_EDIT, BTN_RESET],
        [BTN_START, BTN_HELP],
    ],
...
def build_accounts_summary_text(space_key: str) -> str:
    ...
    for name, balance in rows:
        ...
        if exp_today > 0:
            lines.append(f"{marker}<b>{name}</b>: {fmt_num(balance)} so'm  <i>(bugun xarajat: {fmt_num(exp_today)} so'm)</i>")
        else:
            lines.append(f"{marker}<b>{name}</b>: {fmt_num(balance)} so'm")
    lines.append("—")
    lines.append(f"<b>Umumiy (barcha hisoblar): {fmt_num(total_balance)} so'm</b>")
    if total_today_expense > 0:
        lines.append(f"<i>Bugungi umumiy xarajat: {fmt_num(total_today_expense)} so'm</i>")
    return "\n".join(lines)
...
def build_linked_accounts_text(space_key: str, method: str, title: str) -> str:
    names = get_method_accounts(space_key, method) or []
    rows = dict(get_accounts(space_key))
    lines = [f"<b>{title}</b>"]
    total = 0.0
    for n in names:
        bal = rows.get(n, 0.0)
        ...
        lines.append(f"{marker}<b>{n}</b>: {fmt_num(bal)} so'm")
    lines.append("—")
    lines.append(f"<b>Jami: {fmt_num(total)} so'm</b>")
    return "\n".join(lines)
...
async def send_main_menu(message_obj):
    """Har bir amaldan keyin foydalanuvchini asosiy menyuga qaytarish uchun —
    pastdagi asosiy tugmalar klaviaturasini qayta ko'rsatadi."""
    try:
        await message_obj.reply_text("🏠 Asosiy menyu", reply_markup=MAIN_KEYBOARD)
...
async def show_linked_method(update: Update, context: ContextTypes.DEFAULT_TYPE, method: str, title: str):
    ...
    if not linked:
        ...
        return
    text = build_linked_accounts_text(space_key, method, title)
    buttons = [[InlineKeyboardButton("🔄 Hisoblarni qayta ulash", callback_data=f"methodlink:{method}:relink")]]
    await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(buttons))


async def card_expenses(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await show_linked_method(update, context, "karta", "💳 Karta xarajati bo'yicha ulangan hisoblar")


async def cash_expenses(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await show_linked_method(update, context, "naqt", "💵 Naqt xarajat bo'yicha ulangan hisoblar")
...
async def method_link_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "confirm":
        ...
        text = build_linked_accounts_text(space_key, method, full_title)
        buttons = [[InlineKeyboardButton("🔄 Hisoblarni qayta ulash", callback_data=f"methodlink:{method}:relink")]]
...
def build_excel_report(space_key: str) -> str:
    ...
    r += 1
    if last_data_row >= 2:
        # Formula o'rniga to'g'ridan-to'g'ri hisoblangan qiymat yozamiz —
        # ba'zi Excel til sozlamalarida SUMIFS formulasi vergul/nuqta-vergul
        # farqi tufayli ishlamay qolishi mumkin, shuning uchun ishonchli yo'l
        for currency, offset, total_val in (("UZS", 0, total_uzs), ("USD", 1, total_usd)):
            row_num = r + offset
    ...
    # --- Ikkinchi varaq: kunlik hisoblar qoldig'i (foydalanuvchining o'z hisoblari) ---
    acct_names = get_account_names(space_key)
    ws2 = wb.create_sheet("Hisoblar (kunlik)")
    headers2 = ["Sana"] + acct_names + ["Umumiy"]
    ...
    dates = get_dates_with_activity(space_key)
    r2 = 2
    for date_str in dates:
        balances = get_account_balances_as_of(space_key, date_str)
        ws2.cell(row=r2, column=1, value=date_str).font = normal_font
        row_total = 0.0
        for i, name in enumerate(acct_names, start=2):
            bal = balances.get(name, 0.0)
            row_total += bal
            cell = ws2.cell(row=r2, column=i, value=bal)
            cell.font = normal_font
            cell.number_format = "#,##0.00"
        total_cell = ws2.cell(row=r2, column=len(acct_names) + 2, value=row_total)
        total_cell.font = bold_font
        total_cell.number_format = "#,##0.00"
    ...
    for i in range(len(acct_names) + 1):
        ws2.column_dimensions[get_column_letter(2 + i)].width = 15
    ws2.freeze_panes = "A2"

    # --- Uchinchi varaq: guruhda kim qancha xarajat qilgani (foydalanuvchilar bo'yicha) ---
    user_rows = get_user_totals(space_key)

    if user_rows:
        ws3 = wb.create_sheet("Foydalanuvchilar")
        headers3 = ["Foydalanuvchi", "Valyuta", "Xarajat", "Daromad"]
        ...
        r3 = 2
        for user_id, full_name, currency, xarajat, daromad in sorted(user_rows, key=lambda r: -r[3]):
            display_name = full_name or f"Foydalanuvchi {user_id}"
            ws3.cell(row=r3, column=1, value=display_name).font = normal_font
            ws3.cell(row=r3, column=2, value=currency).font = normal_font
            xc = ws3.cell(row=r3, column=3, value=xarajat)
            xc.font = red_font
            xc.number_format = "#,##0.00"
            dc = ws3.cell(row=r3, column=4, value=daromad)
            dc.font = green_font
            dc.number_format = "#,##0.00"
            for col in range(1, 5):
                ws3.cell(row=r3, column=col).border = border
            r3 += 1
...
async def process_free_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, silent_if_unrecognized: bool = False):
    ...
    # 1f) Hisob nomini o'zgartirish (qayta nomlash) kutilayotgan bo'lsa
    if context.chat_data.get("pending_rename_account"):
        ...
        if ok:
            await update.message.reply_text(f"✅ <b>{esc(old_name)}</b> endi <b>{esc(new_name)}</b> deb nomlandi.\n\n{build_accounts_summary_text(space_key)}")
        else:
            await update.message.reply_text("Bu hisobni topib bo'lmadi.")
        await send_main_menu(update.message)
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(account_management_callback, pattern=r"^acctmgmt:|^acctrename:|^acctdel|^acctaddconfirm:"))
