...
def remember_user(user_id: int, full_name: str):
    ...
    conn.execute(
        ...
        (user_id, full_name or f"Foydalanuvchi {user_id}", datetime.now().isoformat(timespec="seconds")),
    )
    conn.commit()
    conn.close()
...
def log_account_snapshot(space_key: str, name: str, balance: float):
    ...
    conn.execute(
        ...
        (space_key, name, balance, datetime.now().isoformat(timespec="seconds")),
    )
    conn.commit()
    conn.close()
...
async def edit_pick_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    unit = "so'm" if currency == "UZS" else "$"
    await query.edit_message_text(
        f"Joriy summa: <b>{fmt_num(amount)} {unit}</b> — {esc(note)}\n\n"
        f"To'g'ri summani yozing (masalan: 220000):"
    )
...
def build_excel_report(space_key: str) -> str:
    ...
    # --- Ikkinchi varaq: kunlik hisoblar (Uzcard/Humo/Visa/naqt) qoldig'i ---
    ws2 = wb.create_sheet("Hisoblar (kunlik)")
    headers2 = ["Sana"] + ACCOUNT_NAMES + ["Umumiy"]
    ...
    for date_str in dates:
        ...
        row_total = 0.0
        for i, name in enumerate(ACCOUNT_NAMES, start=2):
            bal = balances.get(name, 0.0)
            row_total += bal
            cell = ws2.cell(row=r2, column=i, value=bal)
            cell.font = normal_font
            cell.number_format = "#,##0.00"
        total_cell = ws2.cell(row=r2, column=len(ACCOUNT_NAMES) + 2, value=row_total)
        total_cell.font = bold_font
        total_cell.number_format = "#,##0.00"
    ...
    for i in range(len(ACCOUNT_NAMES) + 1):
...
async def _finalize_commit(update: Update, user_id: int, space_key: str, amount: float, currency: str,
    ...
    text += f"\n<i>{esc(note)}</i>\n\n{format_balance_text(space_key)}\n\n{build_accounts_inline_text(space_key)}"
